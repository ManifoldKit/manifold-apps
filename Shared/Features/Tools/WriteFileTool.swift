...
/// Writes a UTF-8 text file inside the app's sandbox directory.
///
/// App-local on purpose: this is a side-effecting primitive whose safety
/// story depends entirely on the sandbox root the host configures. Lifting it
/// into `ManifoldTools` would invite consumers to wire it without that root
/// and footgun themselves.
///
/// Sandbox policy mirrors `ReadFileTool` / `ListDirTool` (`ManifoldTools`):
...
/// the `ToolApprovalGate` before the file lands on disk — this is the tool
/// the approval-flow UI test exercises.
///
/// Ported from ManifoldKit's `Example/Advanced/WriteFileTool.swift` — logic
/// and tool name (`write_file`) unchanged.
enum WriteFileTool {
    ...
    static func makeExecutor(root: URL) -> any ToolExecutor {
        let definition = ToolDefinition(
            name: "write_file",
            description: "Writes a UTF-8 text file inside the app sandbox. Path must be relative to the sandbox root. Creates parent directories on demand. Requires explicit user approval.",
            parameters: .object([
                "type": .string("object"),
                "properties": .object([
                    "path": .object([
                        "type": .string("string"),
                        "description": .string("Relative path inside the app sandbox.")
                    ]),
                    "content": .object([
                        "type": .string("string"),
