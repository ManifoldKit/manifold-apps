...
/// App-only tools that exercise the unified `ToolResult.ErrorKind`
/// classification + retry behaviour from the orchestration loop.
///
/// These exist purely so the Tools feature can show the framework's error
/// story (invalid args, transient rate limits, MCP failures) end-to-end.
/// Production code should not use them — the fakeRateLimited tool is
/// stateful (failing on the first call and succeeding thereafter) and the
/// fakeMCPLookup tool always fails.
///
/// Both are registered alongside the baseline reference tools by
/// `ManifoldTools.register(on:root:)` (this feature's composition root, not
/// to be confused with the `ManifoldTools` package product).
///
/// Ported from ManifoldKit's `Example/Advanced/FailureDemoTools.swift` —
/// logic and tool names (`fakeRateLimited`, `fakeMCPLookup`) unchanged.
enum FailureTools {

    /// Names of the tools registered by ``register(on:)``. Mirrors
    /// ``ManifoldToolset/baselineNames`` so a future reset-to-defaults path
    /// (e.g. a Scenarios feature) can drop them between scenarios.
...
/// Stateful tool that fails with `.rateLimited` on the first call and
/// succeeds on every subsequent call.
///
/// Demonstrates the orchestrator feeding a transient error back to the model
/// so it can retry within the same turn (governed by
/// `GenerationConfig.maxToolIterations`).
enum FakeRateLimitedTool {
    ...
    /// Mutable counter shared between calls. Wrapped in an actor so the tool
    /// stays Sendable without leaning on locks.
    actor CallState {
...
/// Tool that always fails with `.transient`, simulating an MCP transport
...
enum FakeMCPLookupTool {
    ...
    static func makeExecutor() -> any ToolExecutor {
        let definition = ToolDefinition(
            name: "fakeMCPLookup",
            description: "Looks up a path on a remote MCP service. The remote server is currently unreachable — call once and report the failure rather than looping.",
            parameters: .object([
                "type": .string("object"),
                "properties": .object([
                    "path": .object([
