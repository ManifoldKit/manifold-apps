...
/// Demo intent surfaced through `AppIntentToolExecutor` (`ManifoldAppIntents`)
/// so the model can invoke it during a chat turn.
///
/// Ported near-verbatim from ManifoldKit's own
/// `Example/Advanced/Intents/SetReminderIntent.swift`. The intent doesn't
/// actually wire into EventKit — it logs the request and returns the
/// formatted reminder text. The point is to demonstrate the AppIntent →
/// ToolExecutor bridge (wired in `AppIntentToolsView.swift`), not to ship a
/// real reminder feature.
///
